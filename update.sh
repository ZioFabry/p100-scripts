...
if [ "$(jq .version /home/pi/api/tool/version)" == "0.55" ] ; then
    ...
    curl -s https://raw.githubusercontent.com/ZioFabry/Firmware-script-p100/v0.6.0fix/0.60/EU/update.sh | sudo bash
    wait
...
if [ "$(jq .version /home/pi/api/tool/version)" == "0.6" ] ; then
    ...
    curl -s https://raw.githubusercontent.com/ZioFabry/p100-scripts/main/v060.tuning.sh | sudo bash
else
